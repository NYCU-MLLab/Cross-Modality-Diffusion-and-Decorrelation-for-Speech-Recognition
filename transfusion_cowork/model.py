...
#     def forward(self, x):
#         y = self.W(x.permute(0, 2, 1)).permute(0, 2, 1)
#         return y
...
class Pogfuse(nn.Module):
    ...
    def _sa_block(self, x: Tensor, c: Optional[Tensor], c_padding_mask: Optional[Tensor],
        ...
        if self.add_cond_seq:
            ...
            # print("cccccccc", c.shape)
            # print("xxxxxxxx", x.shape)
            # print("kvkvkvkv", kv.shape)
...
class TransFusion(nn.Module):
    ...
    def forward(self, x: Tensor, t: Tensor, cond_emb: Tensor, cond_padding_mask: Tensor, 
        ...
        for i, layer in enumerate(self.layers):
            ...
            # x, pos_bias,loss = layer(x, t_emb, pooled_cond_emb, cond_emb, x_padding_mask, cond_padding_mask, pos_bias=pos_bias)
            # temp_loss += loss
        # 4. Pass through head to get logits
        x = self.head(x) # (bs, seq_len, vocab size)
        # temp_loss = temp_loss/ len(self.layers)
