...
class PogDataset(Dataset):
    ...
    def __init__(self, df: pd.DataFrame, s2i: dict[str: int], i2s: list | Tensor, T: int, 
        ...
        super().__init__()
        self.df = df
        self.s2i = lambda x: s2i[x] if x in s2i else 33
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, Tensor, Tensor]:
        ...
        # text = []
        # for x in list(row.transcription.strip().upper()):
        #     print(self.s2i.keys())
        #     if x not in self.s2i.keys():
        #         # pritn()
        #         continue
        #     text.append(self.s2i(x))
        # text = [self.s2i(x) if x in self.i2s else self.s2i('!') for x in list(row.transcription.strip().upper())]
