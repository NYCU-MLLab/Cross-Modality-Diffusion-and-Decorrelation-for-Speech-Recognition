...
from count_para import count_parameters
...
def score_model(model: TransFusion, cfg, dl: DataLoader, vocab, dtype, device, restore, ensemble_size, name=None):
    ...
    for i, batch in mb:
        ...
        # I add here
        print("this is targets", text_targets_all)
        print()
        print("this is pred ",text_preds_all)
        for i in range(len(text_targets_all)):
            P, R, F1 = score(text_preds_all, text_targets_all, lang="zh", verbose=True)

        print(f"Bert Sorce is: {F1.mean():.3f}")
        Bert_Sorce.append(F1.mean())
        
        avg = sum(Bert_Sorce)/len(Bert_Sorce)
        print(float(avg))
