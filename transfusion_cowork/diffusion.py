...
class MultinomialDiffusion():
    ...
    def q_pred_one_timestep(self, log_x_t: Tensor, t: Tensor) -> Tensor:
        ...
        return log_probs
    # <kai>
    def q_pred_one_timestep_k(self, log_x_t: Tensor, t: Tensor, k) -> Tensor:
        dt = log_x_t.dtype
        log_alpha_t = extract(self.log_cumprod_alpha, t, log_x_t.shape) - extract(self.log_cumprod_alpha, t - k, log_x_t.shape)
        log_1_min_alpha_t = log_1_min_a(log_alpha_t.to(torch.float64)).to(torch.float32)
        log_alpha_t = log_alpha_t.to(dt)
        log_1_min_alpha_t = log_1_min_alpha_t.to(dt)
        log_probs = log_add_exp(
            log_x_t + log_alpha_t,
            log_1_min_alpha_t - np.log(self.num_classes)
        )
    ...
    # <kai>
    def q_posterior_k(self, log_x_start, log_x_t, t, k):
        t_minus_k = t - k
        t_minus_k = torch.where(t_minus_k < 0, torch.zeros_like(t_minus_k), t_minus_k)
        log_EV_qxtmin_x0 = self.q_pred(log_x_start, t_minus_k)
        num_axes = (1,) * (len(log_x_start.size()) - 1)
        t_broadcast = t.view(-1, *num_axes) * torch.ones_like(log_x_start)
        log_EV_qxtmin_x0 = torch.where(t_broadcast == k - 1, log_x_start, log_EV_qxtmin_x0)
        unnormed_logprobs = log_EV_qxtmin_x0 + self.q_pred_one_timestep_k(log_x_t, t, k)
        log_EV_xtmin_given_xt_given_xstart = \
            unnormed_logprobs \
            - torch.logsumexp(unnormed_logprobs, dim=-1, keepdim=True)
        return log_EV_xtmin_given_xt_given_xstart
    ...
    # <kai>
    def p_pred_k(self, log_x_t, t, log_x0_pred, k):
        log_model_pred = self.q_posterior_k(
            log_x_start=log_x0_pred, log_x_t=log_x_t, t=t, k=k)
        return log_model_pred
    # </kai>
